...
Uma~\deff{fila duplamente terminada} ou fila de duas pontas é uma estrutura de dados que generaliza pilhas e filas. Essa estrutura é uma lista em que é possível adicionar e remover elementos de qualquer uma das suas extremidades. Ela é usualmente chamada de deque, uma abreviatura de dequeue.
...
Note que, usando apenas \funcAPI{PushBack}{d, x}, \funcAPI{Back}{d} e \funcAPI{PopBack}{d}, simulamos uma pilha. Analogamente, usando apenas \funcAPI{PushBack}{d, x}, \funcAPI{Front}{d} e \funcAPI{PopFront}{d}, simulamos uma fila simples.
...
O Código~\ref{lst:deque1_ops1} mostra a implementação das operações mais simples. A função~$\Call{Swap}{d}$ retorna o inverso da deque~$d$ (basta trocar seus dois vértices), e é usada para diminuir a necessidade de duplicar código entre as operações~\API{\textsc{PushFront}} e~\API{\textsc{PushBack}}, assim como~\API{\textsc{PopFront}} e~\API{\textsc{PopBack}}.
