...
\Require $u$ deve ser ativo ou sua cópia deve ser ativa.
\Function{Child}{$u, \side$}
	\If{$u.\ccopy \neq \Null$}
		\State \Return $u.\ccopy.\child[\side]$
	\ElsIf{$u.\eTs \neq -1 \And u.\eSide = \side$}
		\State \Return $u.\extra$
	\Else
		\State \Return $u.\child[\side]$
	\EndIf
...
\Function{Insert}{$\val, \version$}
	\State $\roots[\version] = \roots[\version - 1]$
	\State $x = \New \Call{Node}{\val}$ \Comment{Nó vermelho com valor $\val$ e outros campos vazios.} \label{line:insrb:insb}
	\State $x.\ts = \version$
	\If{$\roots[\version] = \Null$}
		\State $\roots[\version] = x$
	\Else
		\State $u = \roots[version]$
		\While{$u \neq \Null$} \Comment{$v$ é o pai de~$u$.} \label{line:insrb:whilefind}
			\State $v = u$
			\State $u = \Call{Child}{u, [\val > u.\val], \version}$
		\EndWhile
		\State \Call{Modify}{$v, [\val > v.\val], x, \version$}
	...
	\While{$x.\red \And x.\parent \neq \Null \And x.\parent.\red$} \label{line:insrb:loopb}
		...
		\State $z = y.\parent$
		\State $\V{sideX} = [x = \Call{Child}{y, 1, \version}]$
		\State $\V{sideY} = [y = \Call{Child}{z, 1, \version}]$ \label{line:insrb:xyzwe}
		...
		\Else
			\If{$\V{sideX} \neq \V{sideY}$} \Comment{Caso 3.} \label{line:insrb:c3b}
				\State \Call{Rotate}{$y, \V{sideX}, \version$}
				\State $x, y = y, x$ \Comment{Trocando~$x$ e~$y$.} \label{line:insrv:c3e}
			\EndIf
			\State \Call{Rotate}{$z, \V{sideY}, \version$} \Comment{Caso 2.} \label{line:insrb:c2b}
			\If{$z.\ccopy \neq \Null$} \label{line:insrb:if1}
				\State $z = z.\ccopy$
			\EndIf
			\State $z.\red = \True$
			\If{$y.\ccopy \neq \Null$} \label{line:insrb:if2}
				\State $y = y.\ccopy$
			\EndIf
			\State $y.\red = \False$
		...
		\EndIf
	\EndWhile
	\State $\roots[\version].\red = \False$ \Comment{Caso~$x$ seja a raiz, e vermelho.} \label{line:insrb:loope}
\EndFunction
\end{algorithmic}
\caption{Inserção em árvore rubro-negra parcialmente persistente.} \label{lst:ins_rb}
\end{algorithm}

O Código~\ref{lst:ins_rb} mostra então a implementação da inserção, como discutida, dado que a função~\textsc{Rotate} funciona corretamente. As linhas~\nref{line:insrb:insb}-\nref{line:insrb:inse} inserem o nó na árvore como em uma ABB normal, mas usando as funções~\textsc{Child} e~\textsc{Modify} para manter a persistência. As linhas~\nref{line:insrb:loopb}-\nref{line:insrb:loope} fazem o laço discutido nesta seção.

No começo da primeira iteração do laço, as propriedades valem pois~$x$ é um nó vermelho que recém-inserido. As linhas~\nref{line:insrb:xyzwb}-\nref{line:insrb:xyzwe} determinam os nós~$x$,~$y$,~$z$ e~$w$, usados nos casos. O caso~1 é tratado nas linhas~\nref{line:insrb:c1b}-\nref{line:insrb:c1e}, trocando as cores de~$z$,~$y$ e~$w$ e fazendo o~$x$ da próxima iteração ser~$z$, pois é o único nó que possivelmente viola alguma propriedade.

O caso~3 é transformado no caso~2 nas linhas~\nref{line:insrb:c3b}-\nref{line:insrv:c3e}, fazendo uma rotação de~$y$ na direção de~$x$, e trocando esses~2 nós. As linhas~\nref{line:insrb:c2b}-\nref{line:insrb:c2e} então tratam o caso~2, fazendo uma rotação de~$z$ em direção à~$y$ e trocando a cor destes, como na Figura~\ref{fig:rb_ins_caso2}. Os~\keyword{if}s das linhas~\nref{line:insrb:if1} e~\nref{line:insrb:if2} tratam o caso quando as rotações causaram cópias e~$y$ ou~$z$ não são mais ativos, pois para modificar o campo efêmero~$\red$ devemos modificar o nó ativo. Nestes dois casos o~\keyword{break} termina o laço.
...
O Código~\ref{lst:rotrb} mostra a implementação da função~\textsc{Rotate}, que usa apenas um número constante de chamadas a~\textsc{Modify}. Note que é necessário, entre quaisquer duas chamadas, manter que cada vértice tem no máximo um outro vértice que aponta para ele, logo é preciso ter mais cuidado que com a implementação em uma ABB efêmera.

\begin{algorithm}
\begin{algorithmic}[1]
\Require $u$ deve ter um filho~$\side$ na versão~$\version$.
\Function{Rotate}{$u, \side, \version$}
	\State $v = \Call{Child}{u, \side, \version}$
	\If{$u.\parent \neq \Null$}
		\State \Call{Modify}{$u.\parent, [\Call{Child}{u.\parent, 1, \version} = u], v, \version$}
	\EndIf
	\State $\beta = \Call{Child}{v, \Not\ \side, \version}$
	\State \Call{Modify}{$v, \Not\ \side, \version$}
	\State \Call{Modify}{$u, \side, \beta$}
	\State \Call{Modify}{$v, \Not\ \side, u, \version$}
\EndFunction
\end{algorithmic}
