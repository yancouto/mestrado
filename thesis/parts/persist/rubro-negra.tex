...
Vamos apresentar a operação de~\textsc{Find}, que retorna o valor buscado, ou~\keyword{null} se ele não foi encontrado. Se guardarmos um vetor~$\roots$ com os nós de entrada (as raízes), para encontrar um valor~$x$ na versão~$\version$, basta chamar a função~$\Call{Find}{\roots[\version], \version}$.
