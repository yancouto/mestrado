...
\Function{Find}{$x, \version$}
	...
	\If{$u = \Null$}
		\State \Return \Null
	\Else
		\State \Return $u.\val$ % TODO mudar para u e adaptar o resto
	\EndIf
\EndFunction
...
Note que, na linha~\nref{line:copyrb:side} do código da próxima página, devemos usar~$[\Call{Child}{v, 1} = u]$ e não~$[u'.\val > v.\val]$ pois o segundo predicado não funciona quando a ABB pode armazenar valores repetidos, já que~$v$ pode ter~dois filhos com o mesmo valor.
...
A função~\Call{MinElement}{$u$} retorna um nó com menor valor da subárvore de~$u$. Pelas propriedades de uma ABB, este está na subárvore esquerda de~$u$ se esta existe, então o~\keyword{while} segue links de esquerda até encontrar o menor elemento.
