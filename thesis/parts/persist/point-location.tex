...
\Function{Preprocess}{$\{P_1, \ldots, P_k\}$}
	\State Rotacione todos os polígonos por um ângulo~$\alpha$ aleatório. \label{line:prepl:rot} \Comment{$\Oh(n)$}
	\State $\events = \{\}$ \Comment{Vetor vazio}
	\For{$i \in [k]$} \label{line:prepl:for1}
		\For{$j \in [|P_i|]$} \label{line:prepl:for2}
			...
			\State $\seg.\from = a$
			\State $\seg.\tto = b$
			\State $\events.\Call{Add}{\Call{Event}{a.x, \seg}}$ \Comment{Evento de adição}
			\State $\events.\Call{Add}{\Call{Event}{b.x, \seg}}$ \label{line:prepl:fore} \Comment{Evento de remoção}
		\EndFor
	\EndFor
	\State Ordene~$\events$ de forma que~${\events[i].x \leq \events[j].x}$ se~${i < j}$. \Comment{$\Oh(n \lg n)$} \label{line:prepl:ord}
	...
	\For{$e \in \events$} \label{line:prepl:for3}
		\If{$e.x = e.\seg.\from.x$} \Comment{Adicionar segmento}
			\State $\rbt.\Call{Insert}{e.\seg}$ \Comment{$\Oh(\lg n)$}
		\Else \Comment{Remover segmento}
			\State $\rbt.\Call{Remove}{e.\seg}$ \Comment{$\Oh(\lg n)$}
		\EndIf
...
Observe o Código~\ref{lst:prepl}. A linha~\nref{line:prepl:rot} rotaciona todos polígonos por um mesmo ângulo aleatório. Os \keyword{for}s das linhas~\nref{line:prepl:for1} e~\nref{line:prepl:for2} iteram por todos os segmentos dos polígonos. Como os polígonos são dados em sentido horário, então se o segmento vai da esquerda para a direita, os pontos imediatamente acima dele não pertencem ao polígono, e se vai da direita para a esquerda, então pertencem. As linhas~\mbox{\nref{line:prepl:forb}-\nref{line:prepl:fore}} então criam um objeto de segmento corretamente e adiciona dois eventos à lista~$\events$, um para adicionar e outro para remover o segmento.

A lista~$\events$ tem todos os eventos de adição e remoção de segmentos. A linha~\nref{line:prepl:ord} ordena esses eventos pela ordem que eles ocorrem. Dessa forma o~\keyword{for} da linha~\nref{line:prepl:for3} itera pelos segmentos da esquerda para a direita. Note que um evento~$e$ é de adição se~${e.x = e.\seg.\from.x}$ e de remoção caso contrário. Usamos uma árvore rubro-negra parcialmente persistente, com a mesma API que a do Capítulo~\ref{cap:rubronegra_persist}.
...
\Function{WhichPolygon}{p}
	...
	\State Determine o último par~$(x, \current)$ tal que~$x \leq p.x$. \Comment{Busca binária~$\Oh(\lg n)$} \label{line:conpl:bb}
	\State $u = \rbt.\roots[\current]$ \label{line:conpl:ceilb}
	\State $\below = \Null$
	\While{$u \neq \Null$}
		\If{$u.\val.\Call{HeightAt}{p.x} < p.y$} \Comment{Se~$p$ está acima do segmento}
			\State $\below = u.\val$
...
A linha~\nref{line:conpl:rot} rotaciona~$p$ pelo mesmo ângulo que os polígonos foram rotacionados. A linha~\nref{line:conpl:bb} determina a faixa a qual pertence~$p$, usando busca binária na lista~$\slabs$.

Nas linhas~\mbox{\nref{line:conpl:ceilb}-\nref{line:conpl:ceile}}, buscamos o segmento imediatamente abaixo do ponto~$p$. Para isso usamos as funções apresentadas no Capítulo~\ref{cap:rubronegra_persist} e a função~\textsc{HeightAt} da classe segmento, que determina qual a coordenada~$y$ de um segmento em um dado~$x$. O procedimento é análogo ao procedimento de~\textsc{Floor}, descrito por Sedgewick~\cite{Sedgewick}. Com este segmento, podemos determinar o polígono a qual~$p$ pertence (linhas~\mbox{\nref{line:conpl:retb}-\nref{line:conpl:rete}}).
