...
\end{tabular}
	\caption{Consumo de tempo e espaço da implementação de uma pilha de acesso aleatório persistente, usando cada uma das implementações de Ancestral de Nível, onde~$n$ é o tamanho atual da pilha. \label{tab:pilha_persist}} % TODO esta junto com o exemplo que não devia ser table
\end{table}
...
\begin{itemize}
	\item \funcAPI{Queue}{} --- Devolve uma fila vazia.
	\item \funcAPI{Enqueue}{q, x} --- Devolve uma fila igual a~$q$ com o valor~$x$ inserido no seu começo.
	\item \funcAPI{Last}{q} --- Devolve o último elemento da fila~$q$.
	\item \funcAPI{k-th}{q, k} --- Devolve o~\mbox{$k$-ésimo} elemento da fila~$q$.
	\item \funcAPI{Dequeue}{q} --- Devolve uma fila igual a~$q$ com seu último elemento removido.
\end{itemize}

É possível, como com pilhas, implementar uma fila usando lista ligada. A lista torna-se uma árvore quando se adiciona a persistência. É necessário também, para cada versão, armazenar o tamanho atual da fila, ou seja, a fila deixa de ser apenas um nó da árvore e passa a ser um objeto que contém um nó e um inteiro. Seria então necessário usar~Level Ancestor na operação~\funcAPI{k-th}{q, k} ou~\funcAPI{Last}{q}.
...
Sempre assumimos que todas as chamadas de operações são válidas, ou seja, nunca haverá uma chamada para~\funcAPI{k-th}{q, k} com~$k$ maior que o tamanho de~$q$, de modo que os elementos após o final da fila nunca serão acessados indevidamente.
