...
Supomos que, ao adicionar um nó ao conjunto de acessos, todos seus ancestrais já estavam neste conjunto, portanto não existe caminho de um nó não copiado para um nó copiado, logo todos os nós antingíveis a partir da nova versão ou são as cópias criadas ou são nós que nunca foram adicionados ao conjunto de acessos, ou seja, continuam iguais na nova versão. Logo a estrutura ligada funciona e é totalmente persistente. Note que como na Figura~\ref{fig:totfunc_ex} não é possível acessar, a partir de~$a'$, a versão anterior de um nó copiado.

Sem essa suposição, poderia ser possível alcançar a versão antiga de um nó que já foi copiado. Este foi o problema encontrado na implementação da deque de Kaplan e Tarjan no Capítulo~\ref{cap:deque3}, já que na primeira ideia de implementação, era possível acessar a versão antiga de um nó, como discutido na Seção~\ref{sec:implfunc}.

Note que, se os ponteiros da estrutura formam uma floresta direcionada, onde os nós de entrada são as raízes e as arestas vão em direção contrária às raízes, cada nó tem exatamente um caminho até ele, logo a suposição sobre adicionar um nó ao conjunto de acessos é sempre satisfeita, não importa a ordem na qual são feitas as adições. A estrutura discutida no Capítulo~\ref{cap:deque2} é uma árvore direcionada, e na Seção~\ref{sec:implfunc} a estrutura é modificada para se tornar uma árvore direcionada.
...
Para realizar um acesso ao campo~$c$ na versão~$i$, é necessário procurar no vetor associado a~$c$ o par com maior índice que não excede~$i$, ou seja, a última atualização feita até no máximo a versão~$i$. Diferente das outras técnicas, esta também aumenta o tempo de um passo de acesso.
...
%%% MUDAR VARIAVEIS E FAZER ACCESS PARA MODIFICACAO QUE OLHA A COPIA
