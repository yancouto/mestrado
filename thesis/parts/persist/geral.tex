...
\begin{itemize}
	\item $u$ tem uma cópia --- O~\keyword{if} da linha~\nref{line:modify:if1} troca~$u$ por esta cópia; ou
	...
	\item $u$ foi criado nessa versão --- Os~\keyword{if}s iniciais não mudam~$u$.
\end{itemize}

Em qualquer dos casos, após os~\keyword{if}s iniciais,~$u$ é um nó que foi criado nesta versão, e então as linhas~\nref{line:modify:chngb}-\nref{line:modify:chnge} mudam o campo~$\V{field}$.
