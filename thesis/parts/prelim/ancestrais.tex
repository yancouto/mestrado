...
\section{Ancestral de nivel online para árvores}
...
O Código~\ref{lst:lapot2} resolve o problema do Ancestral de Nível online, quando se pode adicionar folhas à árvore. A função~$\Call{AddLeaf}{u}$ consome tempo~$\Oh(\lg \Dep(u))$ e a função~$\Call{LevelAncestor}{k, u}$ consome tempo~$\Oh(\lg k) = \Oh(\lg \Dep(u))$. Note que cada nó~$u$ deve armazenar um vetor de tamanho~$\floor{\lg \Dep(u)}$. Na literatura, essa técnica é chamada de Sparse Table, já que ``armazenamos'' uma tabela de tamanho~$|V| \times |V|$ usando apenas espaço~$|V| \lg |V|$, ou~Jump Pointers, no caso mais específico do Ancestral de Nível em árvores~\cite{BenderM-F2004}. A Figura~\ref{fig:treejmppot2} ilustra os jump pointers de uma árvore.
