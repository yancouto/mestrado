...
Para cada vértice~${u \in V}$, os \deff{ancestrais} de~$u$ são os vértices no (único) caminho de~$u$ até~$r$. O~$(i-1)$-ésimo ancestral de~$u$ é o~$i$-ésimo vértice nesse caminho, ou seja,~$u$ é o $0$-ésimo ancestral de~$u$. Dizemos que o primeiro ancestral de~$u$, se~$u \neq r$, é o \deff{pai} de~$u$. Defina a função~${\Par: V \rightarrow V}$ tal que~$\Par(u)$ seja o pai do vértice~$u$, para todo~$u \in V \setminus \{r\}$; para o vértice~$r$, pode-se considerar que~$\Par(r) = r$. A~\deff{profundidade} de um vértice~$u$ é denotada por~$\Dep(u)$ e é o número de arestas no caminho de~$u$ até~$r$.
...
Assim, temos que~$\Dep(u) = \Dep(v)$ e, para encontrar o ancestral comum mais profundo de~$u$ e~$v$, devemos encontrar o menor~$\kb$ tal que~${\LA(\kb, u) = \LA(\kb, v)}$. Note que se~${k < \Dep(u) - 1}$ e~${\LA(k, u) = \LA(k, v)}$, então
