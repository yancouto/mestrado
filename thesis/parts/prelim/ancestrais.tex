...
\begin{invar}
Ao início da iteração com valor~$i$ do~\keyword{for} da linha~\ref{lst:lcapot2:for} do Código~\ref{lst:lcapot2}, vale que~${0 < \Dep(u) - \Dep(c) \leq 2^{i+1}}$, onde~${c = \LCA(u, v)}$, e o valor de~$\LCA(u, v)$ não se altera ao final da iteração.
\end{invar}
\begin{proof}
